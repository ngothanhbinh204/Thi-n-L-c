...
function createSpinner(text) {
  const quietMode =
    process.argv.includes("--quiet") || process.env.NODE_ENV === "production";
...
try {
  ...
  console.log(
    colors.yellow("⚠️  FTP config not found. FTP deployment disabled.")
  );
}
...
function logBeautiful(type, title, message = "", details = null) {
  ...
  console.log(
    `${timestampStr}${icon} ${titleColor(title)}${
...
function showBuildStats() {
  ...
  const sortedTasks = Object.entries(buildStats.tasks).sort(
    (a, b) => b[1] - a[1]
  );
...
function markBuildSuccess(type) {
  ...
  if (isVerbose) {
    logBeautiful(
      "info",
      "Build Success Marked",
      `${type.toUpperCase()} is ready for deployment`
    );
  }
}
...
function buildFinish(buildTime, showUrls = false) {
  ...
  if (showUrls) {
    ...
    const displayWatchStatus = () => {
      ...
      )}\n${colors.gray("press")} ${colors.cyan("ctrl+c")} ${colors.gray(
        "to exit"
      )}`;
      ...
      if (typeof autoDeployMode !== "undefined") {
        const autoStatus = autoDeployMode
          ? colors.green("on")
          : colors.red("off");
...
async function createFtpConnection() {
  if (!ftpConfig) {
    console.log(
      logSymbols.error,
      "FTP config not loaded. Cannot create connection."
    );
    return null;
  }
...
async function deployFiles(mappingKey) {
  ...
  try {
    ...
    const buildType =
      mappingKey === "styles" ? "css" : mappingKey === "scripts" ? "js" : null;
      ...
      if (spinner)
        spinner.succeed(
          `${colors.cyan("🚀")} ${colors.white(
            mapping.description
          )} ${colors.green("deployed successfully")}`
        );
    } catch (error) {
  ...
  for (const item of items) {
    ...
    if (item.isDirectory()) {
      ...
      try {
        ...
        console.log(
          `${relativePath} - ${fileSize} - ${colors.green("✓")} ${colors.gray(
            "→"
        ...
        console.log(
          `${relativePath} - ${fileSize} - ${colors.red(
            "✗ " + error.message
          )} ${colors.gray("→")} ${colors.cyan(remotePath)}`
        );
        throw error;
      }
    }
...
const buildCoreJS = trackTask("Core JS", async function buildCoreJS() {
  try {
    ...
    if (isVerbose) {
      logBeautiful(
        "success",
        "Core JS built successfully",
        `Bundle created: dist/js/core.min.js`
      );
    }
  ...
  try {
    ...
    if (isVerbose) {
      logBeautiful(
        ...
        `Bundle created: dist/js/main.min.js`
      );
    }
    markBuildSuccess("js");
  } catch (error) {
...
const buildSASS = trackTask("SASS", async function buildSASS() {
  try {
    ...
    if (isVerbose) {
      logBeautiful(
        "success",
        "SASS built successfully",
        `CSS compiled: dist/css/main.min.css`
      );
    }
  ...
  try {
    ...
    filesToProcess.forEach((inputPath) => {
      ...
      try {
        ...
        if (isVerbose) {
          console.log(
            `✅ Compiled: ${fileName} -> ${path.basename(outputPath)}`
          );
        }
      } catch (pugError) {
        console.error(
          `❌ Pug compilation failed for ${fileName}:`,
          pugError.message
        );
      }
    });
...
async function build() {
  ...
  const spinnerText = isFirstBuild
    ? "🚀 Initializing ESBuild process..."
    : "♾️ Rebuilding...";
  ...
  try {
    ...
    if (isFirstBuild || process.argv.includes("--verbose")) {
      ...
      console.log(
        `${colors.green("✓")} ${colors.white("Ready")} ${colors.gray(
          "(" + buildTime + "ms)"
        )}`
      );
    }
...
function initKeyboardShortcuts() {
  if (!ftpConfig) {
    console.log(
      "⚠️  FTP config not loaded. Keyboard shortcuts for deployment disabled."
    );
    return;
  }
  ...
  if (!ftpConfig.shortcuts || typeof ftpConfig.shortcuts !== "object") {
    console.log(
      "⚠️  FTP shortcuts config not found. Keyboard shortcuts disabled."
    );
    return;
  }
  ...
  Object.entries(ftpConfig.shortcuts).forEach(([key, config]) => {
    ...
    shortcutsText += `  ${chalk.bgWhite.black(
      ` ${key.toUpperCase()} `
  ...
  process.stdin.on("data", function (key) {
    ...
    if (shortcut) {
      ...
      console.log(
        `${colors.cyan("⌨️")} ${colors.white(
          "Command received:"
        )} ${colors.yellow(shortcut.description)}`
      );
      ...
      runTask(shortcut.action).catch((error) => {
        console.log(
          `${colors.red("✗")} ${colors.red("Command failed:")} ${error.message}`
        );
      });
    }
  });
  ...
  async function handleExit() {
    ...
    if (activeOperations.size > 0) {
      ...
      if (activeOperations.size > 0) {
        console.log(
          `${colors.yellow("⚠️")} Forcing exit (${
            activeOperations.size
          } operation(s) still active)`
        );
      }
    }
...
async function runTask(taskName) {
  try {
    ...
    console.log(
      `${colors.red("✗")} ${colors.red(taskName + " failed:")} ${error.message}`
    );
  }
}
...
async function autoDeployIfEnabled(fileType, changedPath) {
  ...
  console.log(
    `🚀 Auto-deploy triggered for ${fileType} file: ${path.basename(
      changedPath
    )}`
  );
...
function setupWatchers() {
      ...
      jsTimeout = setTimeout(async () => {
        ...
        try {
          ...
          console.log(
            ...
            )} ${colors.gray("-")} ${colors.cyan("⚙️")} ${colors.yellow(
              compileTime + "ms"
            )}`
          );
        } catch (error) {
      ...
      try {
        ...
        console.log(
          `${colors.magenta("🎨")} ${colors.magenta(
            "SASS changed:"
          )} ${colors.white(path.basename(changedPath))} ${colors.gray(
            "-"
          )} ${colors.cyan("⚙️")} ${colors.yellow(compileTime + "ms")}`
        );
  ...
  chokidar
    ...
    .on("change", async (changedPath) => {
      ...
      if (isVerbose) {
        ...
        console.log(
          `${colors.green("🖼️")} ${colors.green(
            "Image changed:"
          )} ${colors.white(path.basename(changedPath))} ${colors.gray(
            "-"
          )} ${colors.cyan("⚙️")} ${colors.yellow(copyTime + "ms")}`
        );
      }
    })
  ...
  chokidar
    ...
    .on("change", async (changedPath) => {
      ...
      console.log(
        `${colors.yellow("⚙️")} ${colors.yellow(
          "Config changed:"
        )} ${colors.white(path.basename(changedPath))} ${colors.gray(
          "-"
        )} ${colors.cyan("⚙️")} ${colors.yellow(totalTime + "ms")}`
      );
    });
...
function updatePagesTask() {
  try {
    ...
    const currentPagesJson = JSON.parse(
    ...
    if (newPagesCount > 0) {
      outputText(
        "Add File to Pages.json",
        `Added ${newPagesCount} new pages to pages.json`
      );
    } else {
